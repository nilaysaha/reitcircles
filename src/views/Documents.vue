		...
		<v-container>
			<div>
				...
				<div class="tw-text-secondary tw-flex tw-space-x-12 tw-items-center tw-mb-16">
					<h2 class="tw-text-4xl">{{$documents.name}}</h2>
					<span>Personal ID: {{$documents.id}}</span>
				</div>
				<v-tabs background-color="transparent" hide-slider v-model="tabs" grow>
					<v-tab :class="tabs==0? 'tw-bg-0B':'tw-bg-whtie'">
						<!-- <v-btn :class="tabs==0? 'tw-bg-primary':'tw-bg-secondary'" depressed block>Owners Document</v-btn> -->
						<span class="tw-text-2xl" :class="tabs==0? 'tw-text-white':'tw-text-black'">Ownership Document</span>
					</v-tab>

					<v-tab :class="tabs==1? 'tw-bg-0B':'tw-bg-whtie'">
					...
					<v-tab-item class="tw-pt-12">
						<v-card class="tw-bg-transparent" flat>
							<v-card-text class="tw-px-0">
								<v-data-table disable-sort :headers="headers" :items="$documents.documents" hide-default-footer>
										...
										<div class>
											...
											<v-tooltip color="white" top>
												<template v-slot:activator="{ on, attrs }">
													<v-btn class="2xl:tw-ml-20 tw-ml-8" v-bind="attrs" v-on="on" dark color="#0B8793" icon>
								...
								<div class="tw-flex tw-justify-between tw-items-start tw-mt-14">
									<v-btn class="upload-btn-bg" rounded>Upload <v-icon right>mdi-plus-circle-outline</v-icon></v-btn>
									<div class="tw-flex tw-flex-col">
										...
										<div class="tw-text-center status-box tw-mt-4">
											...
											<h2 class="tw-text-secondary tw-text-xl 2xl:tw-text-2xl tw-pt-3">Verification complete</h2>
										</div>
									</div>
								</div>
							</v-card-text>
