...
const validators = {
	...
	name: (v) =>
		_regex.name.test(v) ||
		`Name can only have characters and not more than 32 characters`,
	password: (v) =>
	...
	charAndNumber: (v) =>
		_regex.charAndNumber.test(v) || "Can only have A-Z and numbers.",
	...
	match: (v1, v2, fieldName) =>
		v1 === v2 || `${fieldName || "Field"} doesn't matched.`,
	cantMatch: (v1, v2, fieldName) =>
		v1 !== v2 || `${fieldName || "Field"} can't be same.`,
};
